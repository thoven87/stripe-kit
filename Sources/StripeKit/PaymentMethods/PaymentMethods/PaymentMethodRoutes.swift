...
public protocol PaymentMethodRoutes: StripeAPIRoute {
    ...
    func listAll(
        ...
        type: PaymentMethodType?,
        filter: [String: Any]?
    ) async throws -> PaymentMethodList
    ...
    ///   - expand: Specifies which fields in the response should be expanded.
    /// - Returns: Returns a PaymentMethod object.
    func attach(paymentMethod: String, customer: String, expand: [String]?) async throws
        -> PaymentMethod
...
public struct StripePaymentMethodRoutes: PaymentMethodRoutes {
    ...
    public func create(
        ...
        if let amazonPay { amazonPay.forEach { body["amazon_pay[\($0)]"] = $1 } }
        if let alma { alma.forEach { body["alma[\($0)]"] = $1 } }
        if let billie { billie.forEach { body["billie[\($0)]"] = $1 } }
        if let bizum { bizum.forEach { body["bizum[\($0)]"] = $1 } }
        if let kakaoPay { kakaoPay.forEach { body["kakao_pay[\($0)]"] = $1 } }
        if let krCard { krCard.forEach { body["kr_card[\($0)]"] = $1 } }
        if let mbWay { mbWay.forEach { body["mb_way[\($0)]"] = $1 } }
        if let mobilepay { mobilepay.forEach { body["mobilepay[\($0)]"] = $1 } }
        if let multibanco { multibanco.forEach { body["multibanco[\($0)]"] = $1 } }
        if let naverPay { naverPay.forEach { body["naver_pay[\($0)]"] = $1 } }
        if let nzBankAccount { nzBankAccount.forEach { body["nz_bank_account[\($0)]"] = $1 } }
        if let payByBank { payByBank.forEach { body["pay_by_bank[\($0)]"] = $1 } }
        if let payco { payco.forEach { body["payco[\($0)]"] = $1 } }
        if let paypal { paypal.forEach { body["paypal[\($0)]"] = $1 } }
        if let payto { payto.forEach { body["payto[\($0)]"] = $1 } }
        if let revolutPay { revolutPay.forEach { body["revolut_pay[\($0)]"] = $1 } }
        if let samsungPay { samsungPay.forEach { body["samsung_pay[\($0)]"] = $1 } }
        if let satispay { satispay.forEach { body["satispay[\($0)]"] = $1 } }
        if let sunbit { sunbit.forEach { body["sunbit[\($0)]"] = $1 } }
        if let swish { swish.forEach { body["swish[\($0)]"] = $1 } }
        if let twint { twint.forEach { body["twint[\($0)]"] = $1 } }
        if let upi { upi.forEach { body["upi[\($0)]"] = $1 } }
        if let zip { zip.forEach { body["zip[\($0)]"] = $1 } }
        ...
        return try await apiHandler.send(
            method: .POST, path: paymentmethods, body: .string(body.queryParameters),
            headers: headers)
    }

    public func retrieve(paymentMethod: String, expand: [String]? = nil) async throws
        -> PaymentMethod
    {
        var queryParams = ""
        if let expand {
        ...
        return try await apiHandler.send(
            method: .GET, path: "\(paymentmethods)/\(paymentMethod)", query: queryParams,
            headers: headers
        )
    }
    ...
    public func update(
        ...
        return try await apiHandler.send(
            method: .POST, path: "\(paymentmethods)/\(paymentMethod)",
    ...
    public func listAll(
        ...
        if let type {
            let typeParam = "type=\(type.rawValue)"
            queryParams = queryParams.isEmpty ? typeParam : "\(queryParams)&\(typeParam)"
        }
        if let filter {
            queryParams =
                queryParams.isEmpty
                ? filter.queryParameters : "\(queryParams)&\(filter.queryParameters)"
        }

        return try await apiHandler.send(
            method: .GET, path: paymentmethods, query: queryParams, headers: headers)
    }

    public func attach(paymentMethod: String, customer: String, expand: [String]? = nil)
        async throws
        -> PaymentMethod
    {
        ...
        return try await apiHandler.send(
            method: .POST, path: "\(paymentmethods)/\(paymentMethod)/attach",
            body: .string(body.queryParameters), headers: headers)
    }
    ...
    public func detach(paymentMethod: String, expand: [String]? = nil) async throws -> PaymentMethod
    {
        ...
        return try await apiHandler.send(
            method: .POST, path: "\(paymentmethods)/\(paymentMethod)/detach",
            body: .string(body.queryParameters), headers: headers)
    }
}
