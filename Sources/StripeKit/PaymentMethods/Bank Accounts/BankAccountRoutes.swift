...
public protocol BankAccountRoutes: StripeAPIRoute {
  ...
  func verify(id: String, customer: String, amounts: [Int]?, expand: [String]?) async throws
    -> BankAccount
...
public struct StripeBankAccountRoutes: BankAccountRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(bankaccounts)/\(customer)/sources",
      body: .string(body.queryParameters), headers: headers)
  }

  public func retrieve(id: String, customer: String, expand: [String]? = nil) async throws
    -> BankAccount
  {
    var queryParams = ""
    if let expand {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(bankaccounts)/\(customer)/sources/\(id)", query: queryParams,
      headers: headers)
  }
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(bankaccounts)/\(customer)/sources/\(id)",
      body: .string(body.queryParameters), headers: headers)
  }
  ...
  public func verify(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(bankaccounts)/\(customer)/sources/\(id)/verify",
      body: .string(body.queryParameters), headers: headers)
  }

  public func delete(id: String, customer: String) async throws -> DeletedObject {
    try await apiHandler.send(
      method: .DELETE, path: "\(bankaccounts)/\(customer)/sources/\(id)", headers: headers)
  }

  public func listAll(
    customer: String,
    filter: [String: Any]? = nil
  ) async throws -> BankAccountList {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(bankaccounts)/\(customer)/sources", query: queryParams,
      headers: headers)
  }
}
