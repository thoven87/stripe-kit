...
public struct StripeEarlyFraudWarningRoutes: EarlyFraudWarningRoutes {
  ...
  public func retrieve(earlyFraudWarning: String, expand: [String]? = nil) async throws
    -> EarlyFraudWarning
  {
    var queryParams = ""
    if let expand {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(earlyfraudwarnings)/\(earlyFraudWarning)", query: queryParams,
      headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> EarlyFraudWarningList {
    ...
    return try await apiHandler.send(
      method: .GET, path: earlyfraudwarnings, query: queryParams, headers: headers)
  }
}
