...
public enum EventObject: Codable, Sendable {
    ...
    case billingAlert(BillingAlert)
    case billingCreditGrant(CreditGrant)
    case billingCreditBalanceTransaction(CreditBalanceTransaction)
    case billingMeter(Meter)
    case billingMeterEventAdjustment(MeterEventAdjustment)
    case cashBalanceTransaction(CashBalanceTransaction)
    case confirmationToken(ConfirmationToken)
    case customerSession(CustomerSession)
    case invoiceRenderingTemplate(InvoiceRenderingTemplate)
    case paymentMethodDomain(PaymentMethodDomain)
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "billing.alert":
            self = try .billingAlert(BillingAlert(from: decoder))
        case "billing.credit_grant":
            self = try .billingCreditGrant(CreditGrant(from: decoder))
        case "billing.credit_balance_transaction":
            self = try .billingCreditBalanceTransaction(CreditBalanceTransaction(from: decoder))
        case "billing.meter":
            self = try .billingMeter(Meter(from: decoder))
        case "billing.meter_event_adjustment":
            self = try .billingMeterEventAdjustment(MeterEventAdjustment(from: decoder))
        case "customer_cash_balance_transaction":
            self = try .cashBalanceTransaction(CashBalanceTransaction(from: decoder))
        case "confirmation_token":
            self = try .confirmationToken(ConfirmationToken(from: decoder))
        case "customer_session":
            self = try .customerSession(CustomerSession(from: decoder))
        case "invoice_rendering_template":
            self = try .invoiceRenderingTemplate(InvoiceRenderingTemplate(from: decoder))
        case "payment_method_domain":
            self = try .paymentMethodDomain(PaymentMethodDomain(from: decoder))
        default:
    ...
    public func encode(to encoder: Encoder) throws {
        ...
        case .billingAlert(let alert):
            try alert.encode(to: encoder)
        case .billingCreditGrant(let grant):
            try grant.encode(to: encoder)
        case .billingCreditBalanceTransaction(let transaction):
            try transaction.encode(to: encoder)
        case .billingMeter(let meter):
            try meter.encode(to: encoder)
        case .billingMeterEventAdjustment(let adjustment):
            try adjustment.encode(to: encoder)
        case .cashBalanceTransaction(let transaction):
            try transaction.encode(to: encoder)
        case .confirmationToken(let token):
            try token.encode(to: encoder)
        case .customerSession(let session):
            try session.encode(to: encoder)
        case .invoiceRenderingTemplate(let template):
            try template.encode(to: encoder)
        case .paymentMethodDomain(let domain):
            try domain.encode(to: encoder)
        }
    }
...
public enum EventType: String, Codable, Sendable {
    ...
    /// Occurs whenever balance settings are updated.
    ...
    /// Occurs whenever a Climate order is canceled.
    case climateOrderCanceled = "climate.order.canceled"
    /// Occurs whenever a Climate order is created.
    case climateOrderCreated = "climate.order.created"
    /// Occurs whenever a Climate order is delayed.
    case climateOrderDelayed = "climate.order.delayed"
    /// Occurs whenever a Climate order is delivered.
    case climateOrderDelivered = "climate.order.delivered"
    /// Occurs whenever a Climate order product is substituted.
    case climateOrderProductSubstituted = "climate.order.product_substituted"
    /// Occurs whenever a Climate product is created.
    case climateProductCreated = "climate.product.created"
    /// Occurs whenever a Climate product's pricing is updated.
    ...
    /// Occurs whenever an active entitlement summary is updated.
    case entitlementsActiveEntitlementSummaryUpdated =
    ...
    case financialConnectionsAccountAccountNumbersUpdated =
        "financial_connections.account.account_numbers_updated"
    /// Occurs when a Financial Connections account's ownership data is refreshed.
    case financialConnectionsAccountRefreshedOwnership =
        "financial_connections.account.refreshed_ownership"
    /// Occurs when a Financial Connections account's transaction data is refreshed.
    case financialConnectionsAccountRefreshedTransactions =
        "financial_connections.account.refreshed_transactions"
    /// Occurs when a Financial Connections account number is upcoming to expire.
    case financialConnectionsAccountUpcomingAccountNumberExpiry =
    ...
    /// Occurs whenever an invoice becomes overdue.
    case invoiceOverdue = "invoice.overdue"
    /// Occurs whenever an invoice is overpaid.
    ...
    /// Occurs whenever an invoice payment attempt requires further action.
    ...
    /// Occurs whenever an invoice payment is paid.
    ...
    /// Occurs whenever an invoice will be due.
    ...
    /// Occurs whenever funds are rescinded for an Issuing dispute.
    ...
    /// Occurs whenever a quote will expire.
    case quoteWillExpire = "quote.will_expire"
    ...
    /// Occurs whenever a reserve hold is created.
    case reserveHoldCreated = "reserve.hold.created"
    /// Occurs whenever a reserve hold is updated.
    case reserveHoldUpdated = "reserve.hold.updated"
    /// Occurs whenever a reserve plan is created.
    case reservePlanCreated = "reserve.plan.created"
    /// Occurs whenever a reserve plan is disabled.
    case reservePlanDisabled = "reserve.plan.disabled"
    /// Occurs whenever a reserve plan expires.
    case reservePlanExpired = "reserve.plan.expired"
    /// Occurs whenever a reserve plan is updated.
    case reservePlanUpdated = "reserve.plan.updated"
    /// Occurs whenever a reserve release is created.
    ...
    /// Occurs whenever a Terminal reader action is updated.
