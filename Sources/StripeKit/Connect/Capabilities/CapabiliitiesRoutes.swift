...
public struct StripeCapabilitiesRoutes: CapabilitiesRoutes {
  ...
  public func retrieve(capability: String, expand: [String]? = nil) async throws -> Capability {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(capabilities)/\(capability)/capabilities/card_payments",
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(capabilities)/\(capability)/capabilities/card_payments",
  ...
  public func listAll(account: String) async throws -> CapabilitiesList {
    try await apiHandler.send(
      method: .GET, path: "\(capabilities)/\(account)/capabilities", headers: headers)
  }
}
