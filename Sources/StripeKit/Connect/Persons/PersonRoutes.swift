...
public struct StripePersonRoutes: PersonRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(persons)/\(account)/persons", body: .string(body.queryParameters),
      headers: headers)
  }

  public func retrieve(account: String, person: String, expand: [String]? = nil) async throws
    -> Person
  {
    var body: [String: Any] = [:]
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(persons)/\(account)/persons/\(person)",
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(persons)/\(account)/persons/\(person)",
      body: .string(body.queryParameters), headers: headers)
  }

  public func delete(account: String, person: String) async throws -> DeletedObject {
    try await apiHandler.send(
      method: .DELETE, path: "\(persons)/\(account)/persons/\(person)", headers: headers)
  }
  ...
  public func listAll(account: String, filter: [String: Any]? = nil) async throws -> PersonsList {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(persons)/\(account)/persons", query: queryParams, headers: headers)
  }
}
