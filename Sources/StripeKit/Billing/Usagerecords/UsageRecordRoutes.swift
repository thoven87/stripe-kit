...
public protocol UsageRecordRoutes: StripeAPIRoute {
  ...
  func listAll(subscriptionItem: String, filter: [String: Any]?) async throws
    -> UsageRecordSummaryList
}
...
public struct StripeUsageRecordRoutes: UsageRecordRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(subscriptionitems)/\(subscriptionItem)/usage_records",
      body: .string(body.queryParameters), headers: headers)
  }

  public func listAll(subscriptionItem: String, filter: [String: Any]? = nil) async throws
    -> UsageRecordSummaryList
  {
    var queryParams = ""
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(subscriptionitems)/\(subscriptionItem)/usage_record_summaries",
