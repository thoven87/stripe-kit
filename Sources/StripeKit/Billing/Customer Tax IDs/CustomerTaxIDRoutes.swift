...
public struct StripeCustomerTaxIDRoutes: CustomerTaxIDRoutes {
  ...
  public func create(customer: String, type: TaxIDType, value: String, expand: [String]? = nil)
    async throws -> TaxID
  {
    var body: [String: Any] = [
      "type": type.rawValue,
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(taxids)/\(customer)/tax_ids", body: .string(body.queryParameters),
      headers: headers)
  }
  ...
  public func retrieve(id: String, customer: String, expand: [String]? = nil) async throws -> TaxID
  {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(taxids)/\(customer)/tax_ids/\(id)", query: queryParams,
      headers: headers)
  }

  public func delete(id: String, customer: String) async throws -> DeletedObject {
    try await apiHandler.send(
      method: .DELETE, path: "\(taxids)/\(customer)/tax_ids/\(id)", headers: headers)
  }
  ...
  public func listAll(customer: String, filter: [String: Any]? = nil) async throws -> TaxIDList {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(taxids)/\(customer)/tax_ids", query: queryParams, headers: headers)
  }
}
