...
public struct StripeQuoteLineItemRoutes: QuoteLineItemRoutes {
  ...
  public func retrieve(quote: String, filter: [String: Any]? = nil) async throws
    -> QuoteLineItemList
  {
    var queryParams = ""
    if let filter {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(quotelineitems)/\(quote)/line_items", query: queryParams,
      headers: headers)
  }

  public func retrieveUpfront(quote: String, filter: [String: Any]? = nil) async throws
    -> QuoteLineItemList
  {
    var queryParams = ""
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(quotelineitems)/\(quote)/computed_upfront_line_items",
