...
public struct StripeCustomerBalanceTransactionRoutes: CustomerBalanceTransactionRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(customerbalancetransactions)/\(customer)/balance_transactions",
      body: .string(body.queryParameters), headers: headers)
  }

  public func retrieve(customer: String, transaction: String, expand: [String]? = nil) async throws
    -> CustomerBalanceTransaction
  {
    var queryParams = ""
    if let expand {
    ...
    -> CustomerBalanceTransactionList
  {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(customerbalancetransactions)/\(customer)/balance_transactions",
      query: queryParams, headers: headers)
  }
}
