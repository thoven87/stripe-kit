...
public struct StripeEphemeralKeyRoutes: EphemeralKeyRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: ephemeralkeys, body: .string(body.queryParameters), headers: headers)
  }
  ...
  public func delete(id: String, expand: [String]? = nil) async throws -> EphemeralKey {
    ...
    return try await apiHandler.send(
      method: .DELETE, path: "\(ephemeralkeys)/\(id)/close", body: .string(body.queryParameters),
      headers: headers)
  }
}
