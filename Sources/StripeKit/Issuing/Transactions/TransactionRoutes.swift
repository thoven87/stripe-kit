...
public protocol TransactionRoutes: StripeAPIRoute {
  ...
  func update(transaction: String, metadata: [String: String]?, expand: [String]?) async throws
    -> Transaction
...
public struct StripeTransactionRoutes: TransactionRoutes {
  ...
  public func retrieve(transaction: String, expand: [String]? = nil) async throws -> Transaction {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(issuingtransactions)/\(transaction)", query: queryParams,
      headers: headers)
  }
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(issuingtransactions)/\(transaction)",
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> TransactionList {
    ...
    return try await apiHandler.send(
      method: .GET, path: issuingtransactions, query: queryParams, headers: headers)
  }
}
