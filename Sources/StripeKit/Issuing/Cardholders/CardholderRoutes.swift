...
public struct StripeCardholderRoutes: CardholderRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: cardholders, body: .string(body.queryParameters), headers: headers)
  }
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(cardholders)/\(cardholder)", body: .string(body.queryParameters),
      headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> AuthorizationList {
    ...
    return try await apiHandler.send(
      method: .GET, path: cardholders, query: queryParams, headers: headers)
  }
}
