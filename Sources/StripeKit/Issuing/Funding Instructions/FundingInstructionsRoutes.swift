...
public struct StripeFundingInstructionsRoutes: FundingInstructionsRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: fundinginstructions, body: .string(body.queryParameters),
      headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> FundingInstructionsList {
    ...
    return try await apiHandler.send(
      method: .GET, path: fundinginstructions, query: queryParams, headers: headers)
  }
  ...
  public func simulateTopUp(amount: Int, currency: Currency) async throws -> FundingInstructions {
    ...
    return try await apiHandler.send(
      method: .POST, path: testhelpers, body: .string(body.queryParameters), headers: headers)
  }
}
