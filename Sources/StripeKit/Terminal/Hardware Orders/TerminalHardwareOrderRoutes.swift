...
public protocol TerminalHardwareOrderRoutes: StripeAPIRoute {
  ...
  func preview(
    hardwareOrderItems: [[String: Any]],
    paymentType: TerminalHardwareOrderPaymentType,
    shipping: [String: Any],
    shippingMethod: String,
    metadata: [String: String]?
  ) async throws -> TerminalHardwareOrder
...
public struct StripeTerminalHardwareOrderRoutes: TerminalHardwareOrderRoutes {
  ...
  public func preview(
    ...
    var body: [String: Any] = [
      "hardware_order_items": hardwareOrderItems,
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(terminalhardwareorders)/preview", query: body.queryParameters,
      headers: headers)
  }
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(terminalhardwareorders)/preview", body: .string(body.queryParameters),
      headers: headers)
  }

  public func retrieve(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .GET, path: "\(terminalhardwareorders)/\(order)", headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> TerminalHardwareOrderList {
    ...
    return try await apiHandler.send(
      method: .GET, path: terminalhardwareorders, query: queryParams, headers: headers)
  }

  public func confirmDraftOrder(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .POST, path: "\(terminalhardwareorders)/\(order)/confirm", headers: headers)
  }

  public func cancelOrder(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .POST, path: "\(terminalhardwareorders)/\(order)/cancel", headers: headers)
  }

  public func testMarkReadyToShip(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .POST, path: "\(testhelpers)/\(order)/mark_ready_to_ship", headers: headers)
  }
  ...
  public func testMarkDelivered(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .POST, path: "\(testhelpers)/\(order)/deliver", headers: headers)
  }

  public func testMarkUndeliverable(order: String) async throws -> TerminalHardwareOrder {
    try await apiHandler.send(
      method: .POST, path: "\(testhelpers)/\(order)/mark_undeliverable", headers: headers)
  }
}
