...
public struct StripeTerminalLocationRoutes: TerminalLocationRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: terminallocations, body: .string(body.queryParameters), headers: headers)
  }

  public func retrieve(location: String) async throws -> TerminalLocation {
    try await apiHandler.send(
      method: .GET, path: "\(terminallocations)/\(location)", headers: headers)
  }
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(terminallocations)/\(location)", body: .string(body.queryParameters),
      headers: headers)
  }

  public func delete(location: String) async throws -> DeletedObject {
    try await apiHandler.send(
      method: .DELETE, path: "\(terminallocations)/\(location)", headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> TerminalLocationList {
    ...
    return try await apiHandler.send(
      method: .GET, path: terminallocations, query: queryParams, headers: headers)
  }
}
