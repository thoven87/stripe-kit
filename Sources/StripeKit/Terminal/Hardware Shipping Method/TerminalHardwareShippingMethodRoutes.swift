...
public struct StripeTerminalHardwareShippingMethodRoutes: TerminalHardwareShippingMethodRoutes {
  ...
  private let terminalhardwareshippingmethods =
    APIBase + APIVersion + "terminal/hardware_shipping_methods"
  ...
  public func retrieve(shippingMethod: String) async throws -> TerminalHardwareShippingMethod {
    try await apiHandler.send(
      method: .GET, path: "\(terminalhardwareshippingmethods)/\(shippingMethod)", headers: headers)
  }

  public func listAll(filter: [String: Any]? = nil) async throws
    -> TerminalHardwareShippingMethodList
  {
    var queryParams = ""
    if let filter {
    ...
    return try await apiHandler.send(
      method: .GET, path: terminalhardwareshippingmethods, query: queryParams, headers: headers)
  }
}
