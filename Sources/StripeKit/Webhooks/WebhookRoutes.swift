...
public struct StripeWebhookEndpointRoutes: WebhookEndpointRoutes {
  ...
  public func create(
    ...
    return try await apiHandler.send(
      method: .POST, path: webhooks, body: .string(body.queryParameters), headers: headers)
  }

  public func retrieve(webhookEndpoint: String) async throws -> Webhook {
    try await apiHandler.send(
      method: .GET, path: "\(webhooks)/\(webhookEndpoint)", headers: headers)
  }
  ...
  public func update(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(webhooks)/\(webhookEndpoint)", body: .string(body.queryParameters),
      headers: headers)
  }
  ...
  public func listAll(filter: [String: Any]? = nil) async throws -> WebhookList {
    ...
    return try await apiHandler.send(
      method: .GET, path: webhooks, query: queryParams, headers: headers)
  }

  public func delete(webhookEndpoint: String) async throws -> Webhook {
    try await apiHandler.send(
      method: .DELETE, path: "\(webhooks)/\(webhookEndpoint)", headers: headers)
  }
}
