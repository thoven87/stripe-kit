...
public protocol CashBalanceRoutes: StripeAPIRoute {
  ...
  func retrieveTransaction(customer: String, transaction: String) async throws
...
public struct StripeCashBalanceRoutes: CashBalanceRoutes {
  ...
  public func retrieve(customer: String) async throws -> CashBalance {
    try await apiHandler.send(
      method: .GET, path: "\(cashbalance)/\(customer)/cash_balance", headers: headers)
  }

  public func retrieveTransaction(customer: String, transaction: String) async throws
    -> CashBalanceTransaction
  {
    try await apiHandler.send(
      method: .GET, path: "\(cashbalance)/\(customer)/cash_balance_transactions/\(transaction)",
      headers: headers)
  }
  ...
  public func update(customer: String, settings: [String: Any]? = nil) async throws -> CashBalance {
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(cashbalance)/\(customer)/cash_balance",
    ...
    -> CashBalanceTransactionList
  {
    ...
    return try await apiHandler.send(
      method: .GET, path: "\(cashbalance)/\(customer)/cash_balance_transactions",
  ...
  public func fundCashBalance(
    ...
    return try await apiHandler.send(
      method: .POST, path: "\(testhelper)/\(customer)/fund_cash_balance", headers: headers)
  }
}
